...
class Donation(models.Model):
    ...
    amount = models.DecimalField(max_digits=8, decimal_places=2, blank=True, null=True)
    check_no = models.CharField(max_length=16, blank=True, null=True)
    check_date = models.DateField(max_length=12, blank=True, null=True)
    ...
    def get_absolute_url(self):
        rv = reverse('dbsk:donate_list')
        return rv

    def __str__(self):
        list_representation = ("Amount: %s Check#: %s Date: %s"
                               % (self.amount
                                      , self.check_no
                                      , self.check_date))
        #        print (list_representation)
